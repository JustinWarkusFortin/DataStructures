...
/**The purpose of this class is to declare all the methods that will be called by the child classes as well as 
 * extra methods that check item codes and update quantity in stock as well.
...
public class FoodItem {
	...
	/**
	 * This constructor takes in no parameters and returns nothing.
	 * It initializes all the variables to 0 as well as the itemName to "" 
	 */
	public FoodItem() {
		this.itemCode = 0;
	...
	/**
	 * This toString returns the string of the values that will be entered into the inventory array to be printed out
	 * @return String
	 */
	...
	/**
	 * This method creates two patterns to match to when the user inputs the values for the items they are adding
	 * It prompts the user for input and then checks the condition of a while loop of the pattern desired. If the pattern is not matched,
	 * it prints an error to the user then prompts them again to enter the same value
	 * All inputs are taken in as strings and then parsed into the correct data type.
	 * @param scan Scanner to take in user input
	 * @return boolean will return true no matter what because of Patterns to make sure user input is correct and therefore can never fail
	 */
	...
	/**
	 * This method will take in the quantity of items that will be bought or sold and checks if the itemQuantityInStock will still be above 0 or not
	 * When the method is called and the parameter is entered, the user input is entered as a negative and therefore only 1 calculation is necessary for both
	 * selling and buying
	...
	/**
	 * This method prompts the user to enter an itemCode, the pattern matches it then parses into an int as needed
